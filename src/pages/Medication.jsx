  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="px-5 pt-8 pb-4">
        <h1 className="font-heading text-2xl text-pakistani-green">GLP-1 Medication</h1>
        <p className="text-sm text-muted-foreground mt-1">Track your injection protocol</p>
      </div>
      ...
      {isHalfLifeWindow && (
        <motion.div
          ...
          className="mx-5 mb-4 p-4 rounded-xl bg-dynamic-red/10 border-2 border-dynamic-red"
        >
          <div className="flex items-start gap-3">
            <AlertTriangle className="w-5 h-5 text-dynamic-red flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold text-dynamic-red font-alert text-lg">
                Half-Life Window
              </p>
              <p className="text-xs text-dynamic-red/80 mt-0.5">
      ...
      <div className="px-5 mb-4">
        <div className="bg-shampoo rounded-2xl p-5 border-2 border-pakistani-green">
          <h3 className="font-heading text-lg text-pakistani-green mb-3">Active Protocol</h3>
          {lastDose ? (
            <div className="grid grid-cols-2 gap-3">
              <ProtocolItem
                icon={<Syringe className="w-4 h-4" />}
                label="Last Dose"
                value={`${lastDose.clicks} clicks (${lastDose.mg_dose}mg)`}
              />
              <ProtocolItem
                icon={<Calendar className="w-4 h-4" />}
                label="Days Since"
                value={`${daysSinceLastDose} days`}
              />
              <ProtocolItem
                icon={<Clock className="w-4 h-4" />}
                label="Next Dose"
                value={nextDoseDate?.toLocaleDateString("en-AU", { month: "short", day: "numeric" })}
              />
              <ProtocolItem
                icon="💉"
                label="Site"
                value={lastDose.injection_site}
              />
            </div>
          ) : (
        ...
        {showForm && (
          ...
          >
            <div className="bg-card rounded-2xl p-5 border border-border shadow-sm space-y-4">
              <h3 className="font-heading text-base text-pakistani-green">Log New Injection</h3>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, injection_date: e.target.value }))
                  }
                  className="mt-1"
                />
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, clicks: e.target.value }))
                    }
                    className="flex-1"
                  />
                  <div className="bg-shampoo px-3 py-2 rounded-lg">
                    <span className="text-sm font-semibold text-pakistani-green">
                      = {mgDose.toFixed(2)}mg
                    </span>
                  </div>
                </div>
              ...
              <label className="flex items-start gap-3 p-3 rounded-lg border-2 border-pakistani-green bg-tea-green/30">
                <Checkbox
                  checked={formData.sterility_confirmed}
                  onCheckedChange={(c) =>
                    setFormData((p) => ({ ...p, sterility_confirmed: c }))
                  }
                  className="mt-0.5"
                />
                <div>
                  ...
                  <p className="text-xs text-pakistani-green/70">
                    I confirm clean hands, swabbed site, and new needle.
                  </p>
                </div>
              </label>
              ...
              <div className="flex gap-3">
                ...
                <Button
                  onClick={handleLog}
                  disabled={!canLog}
                  className="flex-1 h-12 bg-electric-blue hover:bg-blue-700 text-white disabled:opacity-40"
                >
                  Log Injection
                </Button>
              </div>
